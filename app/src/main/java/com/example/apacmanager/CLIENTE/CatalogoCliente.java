...
public class CatalogoCliente extends AppCompatActivity {

    private MaterialToolbar toolbar;
    private TextInputEditText etBuscarProducto;
    private RecyclerView recyclerCatalogoCliente;
    private TextView tvNoProductosCliente;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (empresaId == null) {
            Toast.makeText(this, "Error: empresa no identificada", Toast.LENGTH_SHORT).show();
            finish();
            return;
        }
        ...
        recyclerCatalogoCliente = findViewById(R.id.recyclerCatalogoCliente);
        tvNoProductosCliente = findViewById(R.id.tvNoProductosCliente);
        ...
        recyclerCatalogoCliente.setLayoutManager(new GridLayoutManager(this, 2));
        adapter = new ProductoAdapter();
        recyclerCatalogoCliente.setAdapter(adapter);
        ...
        FirebaseDatabaseHelper.getInstance().getProductosReference(empresaId)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onCancelled(@NonNull DatabaseError error) {
                        progressDialog.dismiss();
                        Toast.makeText(CatalogoCliente.this, "Error: " + error.getMessage(), Toast.LENGTH_LONG).show();
                    }
                });
    }
    ...
    private void filtrarProductos(String query) {
        ...
        tvNoProductosCliente.setVisibility(listaFiltrada.isEmpty() ? View.VISIBLE : View.GONE);
    }

    // Adaptador para el catálogo del cliente
    private class ProductoAdapter extends RecyclerView.Adapter<ProductoAdapter.ProductoViewHolder> {
        ...
        public void onBindViewHolder(@NonNull ProductoViewHolder holder, int position) {
            ...
            holder.itemView.setOnClickListener(v -> {
                Intent intent = new Intent(CatalogoCliente.this, DetalleProducto.class);
                intent.putExtra("empresa_id", empresaId);
                intent.putExtra("usuario_uid", usuarioUid);
                intent.putExtra("producto_id", p.getProductoId());
                intent.putExtra("producto_nombre", p.getNombre());
                intent.putExtra("producto_precio", p.getPrecio());
                intent.putExtra("producto_stock", p.getStock());
                intent.putExtra("producto_descripcion", p.getDescripcion());
                intent.putExtra("producto_disponible", p.isDisponible());
